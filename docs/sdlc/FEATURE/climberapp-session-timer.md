...
- **Control shape and interaction states**: `TimerDialog` presents a single toggling button for Start/Stop (label swaps on tap; one 162px-tall `FilledButton`, not two simultaneously-visible buttons) followed by the unchanged Save/Cancel split after Stop. The clock text style and the toggle button are implemented as shared components (`elapsedClockTextStyle`, `TimerToggleButton` in `lib/widgets/timer_toggle_button.dart`) also consumed by `QuickMeasureScreen`, so the two surfaces' shared elements cannot independently drift. This shape, the shared-component relationship, and hover/press/disabled-tap/keyboard-focus behavior (default framework behavior only, no custom interaction affordances, deliberate green/red exception for Save/Cancel) are governed by the `timer-toggle-control` OpenSpec capability (`openspec/specs/timer-toggle-control/spec.md`), which supersedes the earlier dual always-visible-button direction (history in `openspec/changes/archive/2026-08-23-single-toggle-timer-control/` and `openspec/changes/quick-screen-visual-parity/`). This document's Timer Dialog Phase State Machine (Section 4) and Time and Save a Run flow (Section 2) remain the authoritative source for phase transitions; the toggle-control spec governs only the widget shape, shared components, and interaction states within those phases. Once the run is stopped, the frozen duration shown alongside Save/Cancel additionally supports long-press-to-copy (the shared `CopyableRunTime` widget in `lib/widgets/copyable_run_time.dart`): a standard long-press copies the exact displayed value to the clipboard and shows a brief, self-dismissing confirmation; this is inactive on the live-updating clock before Stop. See `openspec/specs/run-time-clipboard-copy/spec.md` (`openspec/changes/copy-run-time-to-clipboard/`).
...
- [ ] The timer overlay shows a single enlarged toggling Start/Stop control while idle/running and reveals Save/Cancel only after Stop (`test/ui_flows_test.dart`)
